...
# Add CORS middleware. Keep this aligned with an automatically reassigned
# frontend port instead of silently allowing only the historical 8501 value.
try:
    _cors_frontend_port = int(os.getenv("FRONTEND_PORT", "8501"))
except ValueError:
    _cors_frontend_port = 8501
if not 1 <= _cors_frontend_port <= 65535:
    _cors_frontend_port = 8501
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        f"http://localhost:{_cors_frontend_port}",
        f"http://127.0.0.1:{_cors_frontend_port}",
    ],
...
class AISettings(BaseModel):
    ...
    provider: str  # Local, vendor API, or OpenAI-compatible gateway
    api_key: str = ""
    ...
    base_url: str = ""  # OpenAI-compatible service root, e.g. https://host/v1


class AIConnectionTest(BaseModel):
    """Ephemeral provider credentials used only for a connection test."""
    provider: str
    api_key: str = ""
    model_name: str = ""
    ollama_url: str = ""
...
async def runtime_diagnostics(refresh_tools: bool = False):
    """Protected operational preflight without exposing configured secrets."""
    missing_tools = (
        orchestrator.refresh_tool_inventory()
        if refresh_tools else list(getattr(orchestrator, "_missing_tools", []))
    )
    db_ok = False
    db_error = None
    try:
        with sqlite3.connect(orchestrator.db_path, timeout=3) as conn:
            conn.execute("SELECT 1").fetchone()
        db_ok = True
    except sqlite3.Error as exc:
        db_error = str(exc)

    required_missing = [tool for tool in ("nmap", "curl") if tool in missing_tools]
    provider = getattr(ai_connector, "provider", None) or os.getenv("AI_PROVIDER", "unconfigured")
    return {
        "status": "ready" if db_ok and not required_missing else "degraded",
        "version": APP_VERSION,
        "database": {"ok": db_ok, "error": db_error},
        "ai_provider": provider,
        "required_tools_missing": required_missing,
        "optional_tools_missing": [tool for tool in missing_tools if tool not in required_missing],
...
async def list_session_history():
    ...
    Kept outside /api/sessions/{session_id} so the static collection route can
    never be interpreted as a session identifier by the router."""
...
async def complete_session(session_id: str):
    ...
    the session archived (it will appear in /api/session-history but not be
...
async def update_ai_settings(settings: AISettings):
    ...
    Supports Ollama, DeepSeek, OpenAI/ChatGPT, Anthropic Claude, OpenRouter,
    ...
    if provider_code not in {"local", "deepseek", "openai", "anthropic", "openrouter", "compatible", "none"}:
    ...
    if provider_code in {"deepseek", "openai", "anthropic", "openrouter", "compatible"}:
        key_name = {
        ...
        if provider_code == "compatible":
            if not settings.base_url:
                raise HTTPException(status_code=400, detail="Base URL is required for OpenAI-Compatible API")
            try:
                from ai.connector import _chat_completions_url
                _chat_completions_url(settings.base_url)
            except ValueError as exc:
                raise HTTPException(status_code=400, detail=str(exc)) from exc
            set_key(env_path, "COMPATIBLE_BASE_URL", settings.base_url.rstrip("/"))
            os.environ["COMPATIBLE_BASE_URL"] = settings.base_url.rstrip("/")
...
async def test_ai_settings(settings: AIConnectionTest):
    """Test credentials/model without saving them or replacing the live connector."""
    provider_map = {
        "Local (Ollama)": "local", "DeepSeek API": "deepseek",
        "OpenAI / ChatGPT API": "openai", "Anthropic Claude API": "anthropic",
        "OpenRouter API": "openrouter", "OpenAI-Compatible API": "compatible",
    }
    provider_code = provider_map.get(settings.provider, settings.provider.strip().lower())
    if provider_code not in {"local", "deepseek", "openai", "anthropic", "openrouter", "compatible"}:
        raise HTTPException(status_code=400, detail="Unsupported AI provider")
    if provider_code == "compatible":
        if not settings.base_url:
        ...
        try:
            from ai.connector import _chat_completions_url
            normalized_url = _chat_completions_url(settings.base_url)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    else:
        normalized_url = None
    try:
        probe = OmitestAIConnector(
            ...
            api_model=settings.model_name or None,
            api_base_url=settings.base_url or None,
        )
        if normalized_url:
            probe.api_urls["compatible"] = normalized_url
        return await probe.test_connection()
    except (ValueError, ConnectionError, httpx.HTTPError, KeyError, json.JSONDecodeError) as exc:
...
def start_operation():
    ...
    try:
        ...
        port = 6000
    if not 1 <= port <= 65535:
        logger.warning("Invalid BACKEND_PORT=%r; using 6000", os.getenv("BACKEND_PORT"))
        port = 6000

    logger.info("Starting omitest backend server...")
    ...
    if not 1 <= frontend_port <= 65535:
        frontend_port = 8501
    logger.info(f"Streamlit Frontend: http://localhost:{frontend_port}")
