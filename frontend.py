...
# Terminal/Brew shell theme. The project-local background is embedded so it
# works identically behind Streamlit on macOS, Linux, systemd, and containers.
_background_path = Path(__file__).resolve().parent / "assets" / "omitest-kali-terminal-bg.png"
try:
    _background_data = base64.b64encode(_background_path.read_bytes()).decode("ascii")
except OSError:
    _background_data = ""
...
<style>
    :root {
        --term-bg: #05070a;
        --term-panel: rgba(7, 14, 22, .91);
        --term-panel-2: rgba(9, 23, 34, .88);
        --term-line: rgba(61, 214, 196, .22);
        --term-green: #3dd6c4;
        --term-green-bright: #73fbd3;
        --term-blue: #60a5fa;
        --term-amber: #f5c56b;
        --term-red: #fb7185;
        --term-text: #d8e7e5;
        --term-muted: #7f9998;
        --term-shadow: 0 18px 50px rgba(0, 0, 0, .28);
    }
    html, body, [class*="css"], .stApp,
    button, input, textarea, select, label, p, li, code {
        font-family: "SFMono-Regular", "Cascadia Code", "Liberation Mono", Menlo, monospace !important;
    }
    .stApp {
        color: var(--term-text);
        background:
            linear-gradient(90deg, rgba(5,7,10,.97) 0%, rgba(5,7,10,.88) 52%, rgba(5,7,10,.72) 100%),
            url("data:image/png;base64,__BACKGROUND__") center / cover fixed,
            var(--term-bg);
    }
    .stApp::before {
        content: ""; position: fixed; inset: 0; pointer-events: none; z-index: 0;
        opacity: .16;
        background-image: repeating-linear-gradient(0deg, transparent 0, transparent 3px, rgba(115,251,211,.045) 4px);
    }
    [data-testid="stHeader"] { background: rgba(5,7,10,.72); backdrop-filter: blur(12px); }
    [data-testid="stToolbar"] { color: var(--term-muted); }
    .block-container { padding: 1.1rem 2rem 4rem; max-width: 1540px; position: relative; z-index: 1; }
    .terminal-chrome {
        display: flex; align-items: center; gap: .55rem; margin: 0 0 1.2rem;
        padding: .72rem 1rem; color: var(--term-muted); font-size: .78rem;
        border: 1px solid var(--term-line); border-radius: 10px;
        background: rgba(5, 11, 17, .88); box-shadow: var(--term-shadow);
    }
    .terminal-chrome .dot { width: 10px; height: 10px; border-radius: 50%; display: inline-block; }
    .terminal-chrome .red { background: #ff5f56; }
    .terminal-chrome .amber { background: #ffbd2e; }
    .terminal-chrome .green { background: #27c93f; }
    .terminal-chrome .prompt { margin-left: .55rem; color: var(--term-green); }
    .main-header {
        font-size: clamp(1.55rem, 3vw, 2.25rem); color: var(--term-green-bright);
        letter-spacing: -.05em; text-align: left; margin: .25rem 0 1.4rem;
        text-shadow: 0 0 24px rgba(61,214,196,.18);
    }
    .main-header::before { content: "$ "; color: var(--term-muted); }
    .sub-header {
        font-size: 1.15rem; color: var(--term-blue); margin: 1.5rem 0 .85rem;
    }
    .session-card, .command-card, .settings-hero, .setup-step,
    [data-testid="stMetric"], [data-testid="stExpander"], [data-testid="stForm"] {
        color: var(--term-text) !important; background: var(--term-panel) !important;
        border: 1px solid var(--term-line) !important; border-radius: 9px !important;
        box-shadow: var(--term-shadow); backdrop-filter: blur(10px);
    }
    .session-card, .command-card { padding: 1rem; margin-bottom: .7rem; border-left: 3px solid var(--term-green) !important; }
    .high-risk { border-left-color: var(--term-red) !important; }
    .medium-risk { border-left-color: var(--term-amber) !important; }
    .low-risk { border-left-color: var(--term-green) !important; }
    .terminal-output {
        background: rgba(1, 6, 9, .96); color: var(--term-green-bright);
        padding: 1rem; border: 1px solid var(--term-line); border-radius: 8px;
        max-height: 440px; overflow-y: auto; white-space: pre-wrap; font-size: .84rem;
        box-shadow: inset 0 0 28px rgba(0,0,0,.45);
    }
    .status-badge {
        padding: .24rem .65rem; border-radius: 4px; font-size: .72rem;
        font-weight: 700; display: inline-block; margin-left: .45rem;
        border: 1px solid currentColor; background: rgba(0,0,0,.32);
    }
    .status-initialized { color: #94a3b8 !important; }
    .status-scanning { color: var(--term-amber) !important; }
    .status-analyzing, .status-ready, .status-completed { color: var(--term-green-bright) !important; }
    .status-executing { color: #c084fc !important; }
    .status-failed { color: var(--term-red) !important; }
    .session-card p, .session-card li, .session-card td, .session-card th,
    .command-card p, .command-card li, .command-card td, .command-card th { color: var(--term-text) !important; }
    .session-card code, .command-card code {
        color: var(--term-green-bright) !important; background: rgba(61,214,196,.08);
    }
    [data-testid="stSidebar"] {
        border-right: 1px solid var(--term-line);
        background: rgba(3, 9, 14, .96); box-shadow: 18px 0 45px rgba(0,0,0,.18);
    }
    [data-testid="stSidebar"] hr { border-color: var(--term-line); }
    [data-testid="stSidebar"] h2 { color: var(--term-green-bright); letter-spacing: -.05em; }
    [data-testid="stTabs"] [data-baseweb="tab-list"] { gap: .35rem; }
    [data-testid="stTabs"] [data-baseweb="tab"] {
        border: 1px solid var(--term-line); border-radius: 6px; padding: .55rem .9rem; height: auto;
        background: rgba(4,11,16,.72); color: var(--term-muted);
    }
    [data-testid="stTabs"] [aria-selected="true"] {
        background: rgba(61,214,196,.11); color: var(--term-green-bright);
    }
    .settings-hero {
        padding: 1.4rem 1.55rem; margin: .2rem 0 1.2rem;
    }
    .settings-hero h1 { margin: 0; font-size: 1.9rem; color: var(--term-green-bright); }
    .settings-hero p { margin: .45rem 0 0; color: var(--term-muted); }
    .setup-step { padding: .9rem 1rem; min-height: 92px; }
    div[data-testid="stButton"] > button, div[data-testid="stFormSubmitButton"] > button {
        border: 1px solid var(--term-green) !important; border-radius: 6px !important;
        color: var(--term-green-bright) !important; background: rgba(7,24,27,.9) !important;
        font-weight: 700; transition: all .16s ease;
    }
    div[data-testid="stButton"] > button:hover, div[data-testid="stFormSubmitButton"] > button:hover {
        color: #03110f !important; background: var(--term-green-bright) !important;
        box-shadow: 0 0 24px rgba(61,214,196,.25); transform: translateY(-1px);
    }
    div[data-baseweb="input"] > div, div[data-baseweb="select"] > div,
    [data-baseweb="textarea"] textarea {
        border: 1px solid var(--term-line) !important; border-radius: 6px !important;
        color: var(--term-text) !important; background: rgba(2,8,12,.9) !important;
    }
    [data-testid="stMetric"] { padding: .8rem 1rem; }
    [data-testid="stMetricLabel"] { color: var(--term-muted); }
    [data-testid="stMetricValue"] { color: var(--term-green-bright); letter-spacing: -.04em; }
    [data-testid="stAlert"] { border-radius: 7px; border: 1px solid var(--term-line); background: rgba(5,13,19,.9); }
    [data-testid="stDataFrame"], [data-testid="stTable"] { border: 1px solid var(--term-line); border-radius: 7px; overflow: hidden; }
    code { color: var(--term-green-bright) !important; }
    a { color: var(--term-blue) !important; }
    hr { border-color: var(--term-line) !important; }
    ::selection { background: rgba(61,214,196,.28); color: white; }
...
st.markdown(
    '<div class="terminal-chrome"><span class="dot red"></span><span class="dot amber"></span>'
    '<span class="dot green"></span><span class="prompt">omitest@kali:~$</span>'
    '<span>./engage --authorized --observe</span></div>',
...
def execute_command(session_id: str, command: str, auto_approve: bool = False):
    ...
    try:
        ...
        request_timeout = max(60, int(os.getenv("COMMAND_TIMEOUT", "600")) + 30)
        response = api_session.post(f"{API_BASE}/execute", json=payload, timeout=request_timeout)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
...
def get_session_history():
    """Get all sessions from DB through the non-overlapping collection route."""
    try:
        response = api_session.get(f"{API_BASE}/session-history", timeout=10)
        if response.status_code == 200:
            return response.json().get("sessions", [])
    except Exception as e:
...
def main():
    ...
    # Sidebar navigation
    with st.sidebar:
        st.markdown("## `>_` omitest")
        st.caption(f"v{APP_VERSION} // authorized operations console")
        st.markdown("---")
        ...
        st.markdown(f"### SYSTEM_LINK {status_color} `{status_text.upper()}`")
        ...
        selected = option_menu(
            ...
            styles={
                ...
                "icon": {"color": "#3dd6c4", "font-size": "17px"},
                "nav-link": {
                    "font-size": "14px", "font-family": "monospace", "text-align": "left",
                    "margin": "3px 0", "border-radius": "5px", "color": "#9bb7b5",
                    "background-color": "transparent",
                },
                "nav-link-selected": {
                    "background-color": "rgba(61,214,196,.12)", "color": "#73fbd3",
                    "border-left": "2px solid #3dd6c4",
                },
            }
        )
        ...
        st.markdown(f"""
        omitest is an AI-driven autonomous red team operator.
...
def show_new_session():
    ...
    with st.form("new_session_form"):
        ...
        authorization_confirmed = st.checkbox(
            ...
            value=False,
            help="Required. omitest will actively scan and may attempt exploitation against this target."
        )
...
<title>omitest Report — {_he(session_details.get('target_ip'))}</title>
<style>
...
<body>
<h1>omitest &mdash; Penetration Test Report</h1>
<table class="meta">
...
<footer>Generated by omitest &mdash; FOR AUTHORISED USE ONLY</footer>
</body></html>"""
...
<div style="text-align:center;padding:80px 20px">
    ...
    <h2 style="color:#90caf9;margin-bottom:8px;font-family:sans-serif">omitest Documentation</h2>
...
def show_settings():
    ...
    <div class="settings-hero">
      <h1>Settings</h1>
      <p>Configure the AI gateway, execution safeguards, and operator experience from one place.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.caption("Choose a provider, verify it live, then save it. Secrets remain in the backend `.env` file.")
        ...
        provider_env = {
            ...
            "OpenAI-Compatible API": ("COMPATIBLE_API_KEY", "COMPATIBLE_MODEL", "gpt-5.4"),
        }
        ...
        with step1:
            st.markdown('<div class="setup-step"><b>1 · Provider</b><br><span style="color:#94a3b8">Select the API protocol.</span></div>', unsafe_allow_html=True)
        with step2:
            st.markdown('<div class="setup-step"><b>2 · Credentials</b><br><span style="color:#94a3b8">Enter a key and model.</span></div>', unsafe_allow_html=True)
        with step3:
            st.markdown('<div class="setup-step"><b>3 · Verify</b><br><span style="color:#94a3b8">Test before saving.</span></div>', unsafe_allow_html=True)
        st.markdown("")

        ai_provider = st.selectbox(
            "AI Provider",
            ["Local (Ollama)", "DeepSeek API", "OpenAI / ChatGPT API", "Anthropic Claude API", "OpenRouter API", "OpenAI-Compatible API"],
            index=["Local (Ollama)", "DeepSeek API", "OpenAI / ChatGPT API", "Anthropic Claude API", "OpenRouter API", "OpenAI-Compatible API"].index(current_label),
        ...
        if ai_provider == "Local (Ollama)":
            ...
            key_configured = bool(env_vars.get(key_env, ""))
            api_key = st.text_input(
                "API key", value="", type="password",
                placeholder="Stored key configured — enter only to replace" if key_configured else "Enter API key",
                help="For security, saved keys are never loaded back into the browser. Leave blank to keep the stored key.",
            )
            cloud_models = {
                ...
                "OpenAI-Compatible API": [
                    "gpt-5.4", "gpt-5.5", "gpt-5.5-xhigh", "gpt-5.6-luna",
                    "gpt-5.6-sol", "gpt-5.6-terra", "claude-opus-5",
                    "claude-opus-5-thinking", "claude-opus-4-8", "claude-opus-4-7",
                    "claude-opus-4-6", "claude-sonnet-5", "claude-sonnet-5-thinking",
                    "claude-sonnet-4-6", "claude-haiku-4-5", "claude-fable-5",
                    "deepseek-v4-pro", "deepseek-v4-flash", "qwen-max", "qwen-plus",
                    "qwen-flash", "qwen-coder-plus", "kimi-k3", "kimi-k2.7-code",
                    "glm-5.3", "glm-5.2", "glm-5.1", "glm-5-turbo",
                    "claude-opus-5-authentic", "claude-opus-4-8-authentic",
                    "claude-opus-4-7-authentic", "claude-opus-4-6-authentic",
                    "claude-sonnet-5-authentic", "claude-sonnet-4-6-authentic",
                    "claude-haiku-4-5-authentic", "gpt-5.5-authentic",
                    "gpt-5.6-luna-authentic", "gpt-5.6-sol-authentic",
                    "gpt-5.6-terra-authentic",
                ],
            }[ai_provider]
            ...
            base_url = ""
            if ai_provider == "OpenAI-Compatible API":
                base_url = st.text_input(
                    "Base URL",
                    value=env_vars.get("COMPATIBLE_BASE_URL", "https://globalblamp.vercel.app/v1"),
                    help="Service root ending in /v1, or a full /chat/completions endpoint.",
                )

            if key_configured:
                st.success("A key is stored for this provider. Entering a new key will replace it.", icon="🔐")
            else:
                st.warning("No key is stored for this provider yet.", icon="🔑")
        ...
        payload = {
            ...
            "base_url": base_url if ai_provider == "OpenAI-Compatible API" else "",
        }

        st.markdown("### Verify and activate")
        test_col, save_col = st.columns(2)
        with test_col:
            test_clicked = st.button("🧪 Test connection", use_container_width=True)
        with save_col:
            save_clicked = st.button("✓ Save and activate", type="primary", use_container_width=True)

        if test_clicked:
            if not check_backend_health():
                st.error("Backend is offline. Start the service and try again.")
            else:
                with st.spinner("Contacting the provider and requesting a small model response…"):
                    try:
                        test_payload = {k: v for k, v in payload.items() if k != "ollama_context_window"}
                        response = api_session.post(
                            f"{API_BASE}/settings/ai/test", json=test_payload, timeout=60
                        )
                        if response.status_code == 200:
                            result = response.json()
                            st.session_state["ai_test_result"] = result
                            st.success(
                                f"Connected to {result.get('provider')} · {result.get('model')} "
                            ...
                            try:
                                ...
                                detail = response.text
                            st.session_state.pop("ai_test_result", None)
                            st.error(f"Connection test failed: {detail}")
        ...
        if save_clicked:
            if not check_backend_health():
                ...
                with st.spinner("Saving configuration..."):
                    try:
                        response = api_session.post(f"{API_BASE}/settings/ai", json=payload, timeout=30)
                        if response.status_code == 200:
                            info = response.json()
                            ctx_info = (
