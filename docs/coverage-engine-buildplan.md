...
1. **Measure first.** Every milestone is judged by the benchmark coverage number
   against the omitest-training-win lab — not by feeling.
...
|---|-----------|--------------|------|
| **M0** | Benchmark harness | `benchmarks/labs/omitest_training_win.json`, `benchmarks/score.py`, tests | baseline number recorded |
...
## Baseline (pre-M1)
Recorded by M0 against the last omitest-training-win report: see
